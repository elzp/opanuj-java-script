...
*/
// posted on https://replit.com/@moreelz4it/Czas-trwania#index.js
const lessons = [
    {
      id: 1,
  ...
  function totalDuration() {
      const resultInMinutes = {all: 0, adam: 0, przemek: 0, marcin: 0}
      const moduleIndicators = [{id:'[1', name: "adam"},{id:'[2', name: 'przemek'}, {id:'[3', name: 'marcin'}]
      lessons.forEach((item)=>{
          const timeValues = item.length.split(":");
          const timeInMinutes = new Number(timeValues[0]) + new Number(timeValues[1]/60);
          moduleIndicators.forEach((item2)=>{
              if(item.name.startsWith(item2.id)){
              resultInMinutes[item2.name] += timeInMinutes;}
          })
          resultInMinutes.all += timeInMinutes;
      })
  
      function getPercent(user){
          if(typeof user !== "string"){ throw new Error("argument must be a string.")}
          return (resultInMinutes[user] / resultInMinutes.all * 100).toFixed(2);
      }
    return `Kurs Opanuj JavaScript trwa ${Math.trunc(resultInMinutes.all/60)} godzin i ${Math.trunc(resultInMinutes.all % 60)} minut.
     Moduł Adama: ${Math.trunc(resultInMinutes.adam/60)} godzin ${Math.trunc(resultInMinutes.adam % 60)} minut, ${getPercent('adam')}%
     Moduł Przemka: ${Math.trunc(resultInMinutes.przemek/60)} godzin ${Math.trunc(resultInMinutes.przemek % 60)} minut, ${getPercent('przemek')}%
     Moduł Marcina: ${Math.trunc(resultInMinutes.marcin/60)} godzin ${Math.trunc(resultInMinutes.marcin % 60)} minut, ${getPercent('marcin')}%
  `;
  }
